...
{
    if (scenarioMohaiminul && fountainsOnM2)
    {
        fountainStageM2++;
...
void updateRedCarKLCC(int value)
{
    if (scenarioMohaiminul)
    {
        redCarKLCC_X += redCarKLCC_Speed;
        if (redCarKLCC_X > 1100)
        {
            redCarKLCC_X = -250;
        }
    }
    glutPostRedisplay();
    glutTimerFunc(16, updateRedCarKLCC, 0);
}
...
void Mohaiminul()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if(fountainsOnM2)
        {
            ...
            // --- STAGE 1: STRAIGHT JETS (Core Water) ---
...
void selectKeyboard(unsigned char key, int x, int y)
{
    ...
    if (scenarioMohaiminul)
    {
        ...
        if(key == 'f' || key == 'F')
        {
            fountainsOnM2 = !fountainsOnM2; // Just switch the flag

            if (fountainsOnM2)
            {
                fountainStageM2 = 0; // Reset animation to start
                // DELETED: updateFountainAnimM2(0);  <-- DO NOT CALL THIS HERE
            }
            glutPostRedisplay();
        }
    }
...
int main(int argc, char** argv)
{
    ...
    glutTimerFunc(20, updateBusKLCC, 0);
    glutTimerFunc(20, updateRedCarKLCC, 0);
